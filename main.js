...
const pay = document.querySelector('.pay-total');
const api = async() =>{
    try {
            ...
            const pushArr = (i,products) =>{
                ...
                btnDelete.addEventListener('click', () =>{
                    // let r = addTrolley[i].id
                    const clk = i+1;
                    // const t = 1
                    // const aux = addTrolley.indexOf('');
                    // const index = addTrolley.indexOf(0);
                    // if (index > -1) {
                    //     addTrolley.splice(index, 1);
                    //   }
                    console.log(i);
                    // console.log(addTrolley.length);
                    // console.log(index);

                    deleteTrolley = addTrolley.slice(clk,1)

                    // deleteTrolley = addTrolley.filter( e => e.id != e[i+1].id)

                    // deleteTrolley = addTrolley.splice(index, 1);
                    console.log(deleteTrolley);
                    // console.log(addTrolley);
                    // console.log(addTrolley);
                    
                });
                console.log(`AVER${addTrolley}`);
                console.log(addTrolley);
                ...
                const id = addTrolley.length;
                if(arrTotal[i] === undefined){
                    arrTotal[i] = 1
                }
                console.log(addTrolley)
                btnAdd.addEventListener('click',()=>{
                    // console.log(`I: ${addTrolley.length}`)
                    arrTotal[i] === 1 ? arrTotal[i] = 2 : arrTotal[i]++;
                    // console.log(arrTotal)
                    let auxTotal = 0;
                    let auxTro
                    arrTotal.map((tro,ind) =>{
                        auxTro = tro * addTrolley[ind].price;
                        totalElement.innerHTML = `<p><b>$ ${auxTro}</b> - ${tro} U`;
                        auxTotal += auxTro;
                        totalPay.innerHTML = " ";
                        totalPay.innerHTML = `<p><b>TOTAL: $ ${auxTotal}</b>`;
                ...
                btnLess.addEventListener('click',()=>{
                    const id = addTrolley.length;
                    console.log(`Antes: ${arrTotal}`)
                    arrTotal[i]  === 0 ? delete addTrolley[i] : arrTotal[i]--;
                    addTrolley[i].productAdd--;
                    // console.log(arrTotal)
                    // console.log(`Despues: ${arrTotal}`)
                    // console.log(arrTotal)
                    let auxTotal = 0;
                    let auxTro
                    arrTotal.map((tro,ind) =>{
